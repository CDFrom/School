...
namespace UserDefinedException
{
    ...
    class Program
    {
        ...
        static bool IsValidName(string str)
        {
            ...
            const String pattern =
                @"^([A-Z][a-z]*\.?\s)*" + //Start with a capital, 0+ lowercase letters, allow for 0-1 '.' followed by a space.
                                          //All of this can occur 0+ times
                @"([A-Z][a-z]+)+$";       //End with a word that begins with a capital and then consists of 1+ lowercase letters.
                                          //All of this can occur 1+ times
        ...
        static bool IsValidPhone(string str)
        {
            ...
            const String pattern =
                @"^(\+?[0-9]{1,4}[-\s])?" + //Optional international section with optional '+', 1-4 digits allowed
                @"([0-9]{3}[-\s]){2}" + //2 sets of 3 numbers
                @"([0-9]{4})$"; //Final set of 4 numbers
